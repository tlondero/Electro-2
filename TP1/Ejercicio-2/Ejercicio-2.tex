...
\begin{equation}
H_{closed loop} = \frac{\Alfa}{1+ f \cdot \Alfa} = \frac{1}{f} \frac{|\Tau|}{1 + |\Tau|}
\end{equation}

donde $\Alfa$ es la respuesta a lazo abierto, $f$ la respuesta del lazo realimentador y $\Tau$ es la ganancia de lazo. Se puede observar que si la ganancia de lazo es mucho mayor a la unidad en módulo, entonces se tiene que 

\begin{equation}
H_{closed loop} \approx \frac{1}{f}
\end{equation}

En el circuito realizado puede observarse un lazo de realimentación negativa el cual posee una inversión de fase producida por el amplificador operacional detallado en la Figura (\ref{fig:circuito_lazo}). También es de 
...
\begin{figure}[H]
\centering
	\includegraphics[width=0.8\textwidth, page=3]{ImagenesEjercicio2/Regulador.pdf}
...
\begin{figure}[H]
\centering
	\includegraphics[width=0.8\textwidth, page=3]{ImagenesEjercicio2/Linearprotection.png}
